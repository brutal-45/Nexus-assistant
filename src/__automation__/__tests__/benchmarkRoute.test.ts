...
describe('benchmarkRoute', () => {
  describe('isBenchmarkRunnerUrl', () => {
    it('matches the bare bench URL', () => {
      expect(isBenchmarkRunnerUrl('nexus://e2e/benchmark')).toBe(true);
    });
    ...
    it('does not match unrelated URLs', () => {
      expect(isBenchmarkRunnerUrl('nexus://chat?palId=foo')).toBe(false);
      expect(isBenchmarkRunnerUrl('nexus://e2e/other')).toBe(false);
    });
  ...
  describe('parseBenchmarkAutostart', () => {
    ...
    it('returns true for autostart=true (case-insensitive)', () => {
      expect(
        parseBenchmarkAutostart('nexus://e2e/benchmark?autostart=true'),
      ).toBe(true);
      expect(
        parseBenchmarkAutostart('nexus://e2e/benchmark?autostart=TRUE'),
      ).toBe(true);
    ...
    it('returns false for autostart=false', () => {
      expect(
        parseBenchmarkAutostart('nexus://e2e/benchmark?autostart=false'),
      ).toBe(false);
    });
    ...
    it('returns false for the bare URL (no query)', () => {
      expect(parseBenchmarkAutostart('nexus://e2e/benchmark')).toBe(false);
    });

    it('returns false when the query has no autostart key', () => {
      expect(parseBenchmarkAutostart('nexus://e2e/benchmark?foo=bar')).toBe(
        false,
      );
    });
    ...
    it('iOS-origin and Android-origin parity: same raw URL resolves identically', () => {
      ...
      const url = 'nexus://e2e/benchmark?autostart=1';
