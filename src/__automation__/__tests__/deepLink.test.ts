...
describe('dispatchAutomationDeepLink', () => {
  ...
  describe('bench host', () => {
    it('navigates to BenchmarkRunner with autostart:false for nexus://e2e/benchmark', async () => {
      const navigate = jest.fn();
      const handled = await dispatchAutomationDeepLink(
        makeParams({
          url: 'nexus://e2e/benchmark',
          host: 'e2e',
        }),
        {navigate},
      );
    ...
    it('navigates with autostart:true for nexus://e2e/benchmark?autostart=1', async () => {
      const navigate = jest.fn();
      const handled = await dispatchAutomationDeepLink(
        makeParams({
          url: 'nexus://e2e/benchmark?autostart=1',
          host: 'e2e',
        }),
        {navigate},
      );
    ...
    it('navigates with autostart:false for nexus://e2e/benchmark?autostart=0', async () => {
      const navigate = jest.fn();
      const handled = await dispatchAutomationDeepLink(
        makeParams({
          url: 'nexus://e2e/benchmark?autostart=0',
          host: 'e2e',
        }),
        {navigate},
      );
