...
The Android `e2e` flavor (`applicationId com.nexusai.nexus.e2e`,
buildType `releaseE2e`) is the production-like build that sets
`E2E_BUILD=true`. The default prod flavor (`com.nexusai.nexus`) leaves
`E2E_BUILD` unset.
...
|--------|---------|------------|
| `BenchmarkRunnerScreen` | Drives the benchmark matrix in-app for the `benchmark-matrix` spec | Deep link `nexus://e2e/benchmark` (registered in `android/app/src/e2e/AndroidManifest.xml`); manual button tap to start |
...
- `memory` — `nexus://memory?cmd=snap::<label>` etc. (memory-profile spec)
