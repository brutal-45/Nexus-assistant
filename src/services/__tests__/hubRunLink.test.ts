...
describe('parseHubRunURL', () => {
  it('parses a valid hub/run link with all params', () => {
    const result = parseHubRunURL(
      'nexus://hub/run?repo_id=author/model&filename=model.Q4_K_M.gguf&source=hf',
    );
  ...
  it('returns null for the wrong path', () => {
    expect(
      parseHubRunURL(
        'nexus://hub/download?repo_id=author/model&filename=x.gguf',
      ),
    ).toBeNull();
  });
  ...
  it('returns null when repo_id has no slash (malformed)', () => {
    expect(
      parseHubRunURL('nexus://hub/run?repo_id=authormodel&filename=x.gguf'),
    ).toBeNull();
  });

  it('returns null when repo_id has an empty half', () => {
    expect(
      parseHubRunURL('nexus://hub/run?repo_id=author/&filename=x.gguf'),
    ).toBeNull();
  });

  it('returns null when repo_id contains path-traversal segments', () => {
    expect(
      parseHubRunURL('nexus://hub/run?repo_id=../x&filename=x.gguf'),
    ).toBeNull();
    expect(
      parseHubRunURL('nexus://hub/run?repo_id=a/..&filename=x.gguf'),
    ).toBeNull();
  });

  it('returns null when repo_id has more than two segments', () => {
    expect(
      parseHubRunURL('nexus://hub/run?repo_id=a/b/c&filename=x.gguf'),
    ).toBeNull();
  });
  ...
  it('accepts a link with no filename (filename is optional)', () => {
    const result = parseHubRunURL(
      'nexus://hub/run?repo_id=author/model&source=hf',
    );
    expect(result).toEqual({
      repoId: 'author/model',
      filename: undefined,
  ...
  it('accepts a non-.gguf filename and keeps it verbatim', () => {
    const result = parseHubRunURL(
      'nexus://hub/run?repo_id=author/model&filename=model.bin',
    );
    expect(result).toEqual({
      repoId: 'author/model',
      filename: 'model.bin',
  ...
  it('keeps a present filename verbatim', () => {
    const result = parseHubRunURL(
      'nexus://hub/run?repo_id=author/model&filename=model.GGUF',
    );
  ...
  it('treats a blank filename param as absent', () => {
    const result = parseHubRunURL(
      'nexus://hub/run?repo_id=author/model&filename=%20%20',
    );
  ...
  it('passes an arbitrary source value through unvalidated', () => {
    const result = parseHubRunURL(
      'nexus://hub/run?repo_id=author/model&filename=model.gguf&source=anything-goes',
    );
  ...
  it('trims whitespace around repo_id and filename', () => {
    const result = parseHubRunURL(
      'nexus://hub/run?repo_id=%20author/model%20&filename=%20model.gguf%20',
    );
    expect(result).toEqual({
      repoId: 'author/model',
      filename: 'model.gguf',
...
describe('isHubLink', () => {
  it('is true for the exact hub/run route regardless of query', () => {
    expect(isHubLink('nexus://hub/run')).toBe(true);
    expect(isHubLink('nexus://hub/run?repo_id=a/b')).toBe(true);
  });

  it('is false for unknown hub paths', () => {
    expect(isHubLink('nexus://hub/foo')).toBe(false);
    expect(isHubLink('nexus://hub')).toBe(false);
  });

  it('is false for non-hub hosts', () => {
    expect(isHubLink('nexus://chat?palId=x')).toBe(false);
    expect(isHubLink('nexus://memory')).toBe(false);
    expect(isHubLink('nexus://e2e/benchmark')).toBe(false);
  });
