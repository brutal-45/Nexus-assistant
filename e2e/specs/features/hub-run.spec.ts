 ...
 *
 * Drives the `nexus://hub/run` deep link end-to-end: fire the link (as the
...
const ANDROID_PACKAGE = 'com.nexusai.nexus.e2e';
const IOS_BUNDLE_ID = 'com.nexusai.nexus';
...
/**
 * Fire a nexus:// deep link via Appium's mobile: deepLink, targeting the
...
describe('Hub Run Deep Link', () => {
  ...
  it('rejects a deep link missing repo_id and opens no sheet', async () => {
    await fireDeepLink('nexus://hub/run?filename=x.gguf');
  ...
  it(`downloads ${model.id} via hub/run deep link, loads, and chats`, async () => {
    ...
    await fireDeepLink(`nexus://hub/run?repo_id=${REPO_ID}&source=hf`);
