...
Drives the in-app **BenchmarkRunnerScreen** via deep link (`nexus://e2e/benchmark`) — no WDIO required for ad-hoc runs. Three tiers gated by `BENCH_TIER`:
...
adb shell am start -a android.intent.action.VIEW \
  -d 'nexus://e2e/benchmark' -p com.nexusai.nexus.e2e

# 3. Tap "Run benchmark matrix" on the screen (or via adb input tap)
# 4. Wait for `bench-runner-screen-status` content-desc == "complete"
# 5. Pull the report
adb pull /sdcard/Android/data/com.nexusai.nexus.e2e/files/benchmark-report-*.json
```
...
# Installs as com.nexusai.nexus.e2e and coexists side-by-side with the
# prod install (com.nexusai.nexus). E2E_BUILD=true is set automatically
# so the automation bridge (src/__automation__/) ships in this APK.
```

**Flavor.** E2E targets the `e2e` flavor (`com.nexusai.nexus.e2e`, debuggable),
...
client entries for both `com.nexusai.nexus` and `com.nexusai.nexus.e2e`. If the
...
APK is non-debuggable, so `adb shell run-as com.nexusai.nexus` and
`adb push` into `/data/data/com.nexusai.nexus/files/…` will not work.
...
```
/data/data/com.nexusai.nexus.e2e/files/models/hf/<author>/<repo>/<filename>.gguf
```

Push each GGUF once:

```bash
adb shell run-as com.nexusai.nexus.e2e mkdir -p \
...
adb shell run-as com.nexusai.nexus.e2e sh -c \
